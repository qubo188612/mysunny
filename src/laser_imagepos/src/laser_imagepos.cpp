  ...
  _handle = this->add_on_set_parameters_callback(
    [this](const std::vector<rclcpp::Parameter> & vp) {
      ...
      for (const auto & p : vp) {
        ...
        else if(alg_return=alg105_getcallbackParameter(p)!=0)
        {
          if(alg_return<0)
          {
            result.successful = false;
            result.reason = "Failed to set alg105_Parameter";
            return result;
          }
        }
        else if(alg_return=alg106_getcallbackParameter(p)!=0)
        {
          if(alg_return<0)
          {
            result.successful = false;
            result.reason = "Failed to set alg106_Parameter";
            return result;
          }
        }
        else if(alg_return=alg107_getcallbackParameter(p)!=0)
        {
          if(alg_return<0)
          {
            result.successful = false;
            result.reason = "Failed to set alg107_Parameter";
            return result;
          }
        }
        else if(alg_return=alg108_getcallbackParameter(p)!=0)
        {
          if(alg_return<0)
          {
            result.successful = false;
            result.reason = "Failed to set alg108_Parameter";
            return result;
          }
        }
      }
...
void LaserImagePos::_declare_parameters()
{
  ...
  alg105_declare_parameters();
  alg106_declare_parameters();
  alg107_declare_parameters();
  alg108_declare_parameters();
...
Params LaserImagePos::_update_parameters()
{
  ...
  alg105_update_parameters();
  alg106_update_parameters();
  alg107_update_parameters();
  alg108_update_parameters();
  return pm;
}
                            ...
                            int step)    //输出结果点信息
{
  ...
  switch(pm.task_num)
  {
    ...
    break;
    case 105:   //搭接
      if(0!=alg105_runimage(imageIn,pointcloud,namepoint,solderjoints,step))
        return 1;
    break;
    case 106:   //坡口
      if(0!=alg106_runimage(imageIn,pointcloud,namepoint,solderjoints,step))
        return 1;
    break;
    case 107:   //坡口
      if(0!=alg107_runimage(imageIn,pointcloud,namepoint,solderjoints,step))
        return 1;
    break;
    case 108:   //STC
      if(0!=alg108_runimage(imageIn,pointcloud,namepoint,solderjoints,step))
        return 1;
    break;
    default:
      return 1;
