...
int32_t MyPlcFunction::Myfixfdata(float *dataIn_Out,u_int8_t *mask,int32_t num)
{
    int32_t n,m;
    int32_t nST,nED;
    int32_t *u32_data=new int32_t[num];
    int32_t seed=0;
    u_int8_t nTemp=0;
    int32_t addnum;
    float add;
    ...
    memset(u32_data,0,num*sizeof(int32_t));
    for(n=0;n<num;n++)
    {
        if(mask[n]==1)
        {
...
void MyPlcFunction::save_pcldata_pclclould(pcl::PointCloud<pcl::PointXYZ>::Ptr pclclould)
{
    ...
    if(pclclould->size() > 0)
    {
      pcl::io::savePCDFile(dir,*pclclould);
    }
    return;
}