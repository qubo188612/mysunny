// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from tutorial_interfaces:msg/IfAlgorhmitcloud.idl
// generated code does not contain a copyright notice
#include "tutorial_interfaces/msg/detail/if_algorhmitcloud__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `lasertrackoutcloud`
#include "tutorial_interfaces/msg/detail/if_algorhmitpoint4f__functions.h"
// Member `targetpointoutcloud`
#include "tutorial_interfaces/msg/detail/if_algorhmittargetpoint4f__functions.h"
// Member `robpos`
#include "tutorial_interfaces/msg/detail/if_algorhmitrobpos__functions.h"

bool
tutorial_interfaces__msg__IfAlgorhmitcloud__init(tutorial_interfaces__msg__IfAlgorhmitcloud * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__fini(msg);
    return false;
  }
  // lasertrackoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmitpoint4f__Sequence__init(&msg->lasertrackoutcloud, 0)) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__fini(msg);
    return false;
  }
  // targetpointoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmittargetpoint4f__Sequence__init(&msg->targetpointoutcloud, 0)) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__fini(msg);
    return false;
  }
  // robpos
  if (!tutorial_interfaces__msg__IfAlgorhmitrobpos__init(&msg->robpos)) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__fini(msg);
    return false;
  }
  // solderjoints
  return true;
}

void
tutorial_interfaces__msg__IfAlgorhmitcloud__fini(tutorial_interfaces__msg__IfAlgorhmitcloud * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // lasertrackoutcloud
  tutorial_interfaces__msg__IfAlgorhmitpoint4f__Sequence__fini(&msg->lasertrackoutcloud);
  // targetpointoutcloud
  tutorial_interfaces__msg__IfAlgorhmittargetpoint4f__Sequence__fini(&msg->targetpointoutcloud);
  // robpos
  tutorial_interfaces__msg__IfAlgorhmitrobpos__fini(&msg->robpos);
  // solderjoints
}

bool
tutorial_interfaces__msg__IfAlgorhmitcloud__are_equal(const tutorial_interfaces__msg__IfAlgorhmitcloud * lhs, const tutorial_interfaces__msg__IfAlgorhmitcloud * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // lasertrackoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmitpoint4f__Sequence__are_equal(
      &(lhs->lasertrackoutcloud), &(rhs->lasertrackoutcloud)))
  {
    return false;
  }
  // targetpointoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmittargetpoint4f__Sequence__are_equal(
      &(lhs->targetpointoutcloud), &(rhs->targetpointoutcloud)))
  {
    return false;
  }
  // robpos
  if (!tutorial_interfaces__msg__IfAlgorhmitrobpos__are_equal(
      &(lhs->robpos), &(rhs->robpos)))
  {
    return false;
  }
  // solderjoints
  if (lhs->solderjoints != rhs->solderjoints) {
    return false;
  }
  return true;
}

bool
tutorial_interfaces__msg__IfAlgorhmitcloud__copy(
  const tutorial_interfaces__msg__IfAlgorhmitcloud * input,
  tutorial_interfaces__msg__IfAlgorhmitcloud * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // lasertrackoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmitpoint4f__Sequence__copy(
      &(input->lasertrackoutcloud), &(output->lasertrackoutcloud)))
  {
    return false;
  }
  // targetpointoutcloud
  if (!tutorial_interfaces__msg__IfAlgorhmittargetpoint4f__Sequence__copy(
      &(input->targetpointoutcloud), &(output->targetpointoutcloud)))
  {
    return false;
  }
  // robpos
  if (!tutorial_interfaces__msg__IfAlgorhmitrobpos__copy(
      &(input->robpos), &(output->robpos)))
  {
    return false;
  }
  // solderjoints
  output->solderjoints = input->solderjoints;
  return true;
}

tutorial_interfaces__msg__IfAlgorhmitcloud *
tutorial_interfaces__msg__IfAlgorhmitcloud__create()
{
  tutorial_interfaces__msg__IfAlgorhmitcloud * msg = (tutorial_interfaces__msg__IfAlgorhmitcloud *)malloc(sizeof(tutorial_interfaces__msg__IfAlgorhmitcloud));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tutorial_interfaces__msg__IfAlgorhmitcloud));
  bool success = tutorial_interfaces__msg__IfAlgorhmitcloud__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
tutorial_interfaces__msg__IfAlgorhmitcloud__destroy(tutorial_interfaces__msg__IfAlgorhmitcloud * msg)
{
  if (msg) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__fini(msg);
  }
  free(msg);
}


bool
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__init(tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  tutorial_interfaces__msg__IfAlgorhmitcloud * data = NULL;
  if (size) {
    data = (tutorial_interfaces__msg__IfAlgorhmitcloud *)calloc(size, sizeof(tutorial_interfaces__msg__IfAlgorhmitcloud));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tutorial_interfaces__msg__IfAlgorhmitcloud__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tutorial_interfaces__msg__IfAlgorhmitcloud__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__fini(tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tutorial_interfaces__msg__IfAlgorhmitcloud__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence *
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__create(size_t size)
{
  tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * array = (tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence *)malloc(sizeof(tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__destroy(tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * array)
{
  if (array) {
    tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__fini(array);
  }
  free(array);
}

bool
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__are_equal(const tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * lhs, const tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tutorial_interfaces__msg__IfAlgorhmitcloud__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence__copy(
  const tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * input,
  tutorial_interfaces__msg__IfAlgorhmitcloud__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tutorial_interfaces__msg__IfAlgorhmitcloud);
    tutorial_interfaces__msg__IfAlgorhmitcloud * data =
      (tutorial_interfaces__msg__IfAlgorhmitcloud *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tutorial_interfaces__msg__IfAlgorhmitcloud__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          tutorial_interfaces__msg__IfAlgorhmitcloud__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tutorial_interfaces__msg__IfAlgorhmitcloud__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
